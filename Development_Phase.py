...
def initializing_tweet_list():  # REFACTOOOOR!!!
    ...
    with open("tweetdhead300000.json", "r") as json_file_read:
        for line in json_file_read:
            ...
            all_tweets.append(
                tweet)  # h all_tweet prosorina einai mia lista apo dictionaries, kai to index tou tweet einai to id tou
...
def save_changes():  # REFACTOOOOOR!!!!!
    ...
    with open("tweetdhead300000.json", "w") as json_to_write:
...
def main():
    ...
    while True:
        ...
        if instruction == "c":
            ...
            read_tweet(
                len(all_tweets) - 1)  # size is by 1 bigger than the sum of indexes cause it starts with 1. If i make up for the
